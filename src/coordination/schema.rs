...
"#,
    ...
    ensure_column(
        ...
        "assignments",
        "profile",
        "ALTER TABLE assignments ADD COLUMN profile TEXT",
    )?;
    ensure_column(
        connection,
        "assignments",
        "dispatched_by",
        "ALTER TABLE assignments ADD COLUMN dispatched_by TEXT",
    )?;
    Ok(())
}
...
///
/// The DDL above is `CREATE TABLE IF NOT EXISTS`, so an existing database keeps
/// its original column set — a newly added column would exist only on a fresh
/// database. `table_info` is the check rather than tolerating a duplicate-column
/// error, so a genuine failure (a locked database, a bad statement) still
/// surfaces instead of being read as "already migrated".
fn ensure_column(
...
mod tests {
    ...
    #[test]
    fn ensure_is_idempotent_and_adds_the_profile_column() {
        let connection = Connection::open_in_memory().unwrap();
        ensure(&connection).unwrap();
        ensure(&connection).unwrap();
        let mut stmt = connection.prepare("PRAGMA table_info(assignments)").unwrap();
    ...
    #[test]
    fn a_legacy_assignments_table_gains_the_column() {
        // Simulates a database created before the column existed: the original
        // columns are present (the indexes over them must still be creatable),
        // and only `profile` is missing.
        let connection = Connection::open_in_memory().unwrap();
        connection
            .execute_batch(
