...
commands = "Available commands: `snippet browser uri --json`, `snippet browser list --json`, `snippet browser call`, `snippet browser snapshot`, `snippet browser click`, `snippet browser type`, `snippet browser drag`, and `snippet browser upload`. There is no separate `screenshot` or `eval` CLI subcommand: use `call --method page.screenshot` or `call --method page.eval`. Use `--device-name NAME`; legacy `--browser NAME` is only a compatibility alias. Do not use a browser ID as the selector."
