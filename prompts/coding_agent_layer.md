...
browser_cli = "Use the [browser_manual] section below for all browser control. Do not improvise browser APIs or direct CDP/WebSocket calls. If unsure, run `snippet browser manual --json` and follow it."
...
start = "Before page actions, run `snippet browser list --json` and use the exact device_name. If empty, stop. When unsure, run `snippet browser manual --json`."
uri = "`snippet browser uri --json` is for configuring the browser extension, not for the coding agent to open, paste into a browser page, or use as a browser-control command. Run it only when you need the authenticated WebSocket URI; copy the returned `uri` into the Snippet Browser extension's options/configuration page. After the extension connects, the agent uses `snippet browser list --json`, `snippet browser call`, and the higher-level browser commands. Never paste the URI into a page or try to navigate to it."
tab_discovery = "Run `snippet browser call --device-name NAME --method tabs.query --args '{}'`. Extract the numeric result[].id as TAB_ID. Never guess, omit, substitute a window ID, or use a snapshot ref as TAB_ID."
call = "`call` relays an allowlisted extension method: `snippet browser call --device-name NAME --method METHOD --args '{...}'`. Common methods include tabs.query/get/create/update/remove, page.snapshot/click/type/key/eval/screenshot, and netwatch.start/pause/resume/getEvents/stop. To navigate, use tabs.update with JSON such as `'{\"tabId\": TAB_ID, \"url\": \"https://example.com\"}'`; do not invent page.navigate, Runtime.evaluate, page.keydown, or page.keyup. Quote JSON as one shell argument."
network = "Network watching is a bounded pull workflow: call `netwatch.start` with `{\"tabId\": TAB_ID}` (optional maxEvents up to 1000), perform the test, call `netwatch.getEvents` (optional limit), then call `netwatch.stop`. Use netwatch.pause/resume when needed. Events contain metadata only — URL, method, resource type, status, errors, and timestamps; never assume request or response bodies are available. Always stop the watcher when finished."
...
drag_coordinates = "Drag with viewport CSS-pixel coordinates using `snippet browser drag-coordinates --device-name NAME --tab TAB_ID --x1 X1 --y1 Y1 --x2 X2 --y2 Y2 [--steps N]`; use this when snapshot refs are unavailable. Coordinates are relative to the visible page viewport; steps are bounded to 2-100."
...
required_tab_id = "Every page.* method requires numeric tabId, including page.eval, page.snapshot, page.click, page.type, page.key, page.drag, page.upload, and page.screenshot. If tabId is missing, stop, run tabs.query, and retry once with the returned ID."
keyboard = "Keyboard uses only page.key: `{\"tabId\": TAB_ID, \"key\": \"Enter\"}`. Never use page.keydown or page.keyup."
evaluation = "JavaScript uses page.eval with `{\"tabId\": TAB_ID, \"expression\": \"...\"}`. Never use Runtime.evaluate. Always include numeric tabId."
recovery = "After one failed browser command, make one changed next action: re-list for connection errors, re-query for tabId errors, re-snapshot for refs, consult `snippet browser manual --json` for method errors, or report the blocker. Never repeat the exact failing call."
